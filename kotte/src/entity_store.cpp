...
namespace kotte
{
    ...
    Entity* EntityStore::try_get(EntityHandle handle) noexcept{
        ...
        Slot& slot = slots_[handle.slot];
        // TODO(Week 6): An occupied slot may belong to a newer lifetime.
        // Complete the validation before returning a borrowed pointer.
    ...
    const Entity* EntityStore::try_get(EntityHandle handle) const noexcept{
        ...
        // TODO(Week 6): Keep this overload's identity rules consistent with
        // the mutable overload above.
        if(!slot.entity.has_value()){
            return nullptr;
        }
